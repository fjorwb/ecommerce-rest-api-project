...
const createOrder = async (request, response) => {
  ...
  if (temp?.length === 0) {
    ...
    for (let i = 0; i < temp1.length; i++) {
      const statement2 =
        'INSERT INTO orders (order_id, product_id, quantity, price, discount, tax, date) VALUES ($1, $2, $3, $4, $5, $6, $7)'
      const values2 = [
        order_id,
        temp1[i].product_id,
        temp1[i].quantity,
    ...
    // CREATE THE ACCOUNT
    const statement4 =
      'INSERT INTO accounts (account_id, user_id, order_id, amount, date, tax, accotype) VALUES($1, $2, $3, $4, $5, $6, $7)'
...
const updateOrder = async (request, response) => {
  const { order_id, product_id, quantity, price, discount, tax, date } = request.body
